...
import { JobApplication, Feedback, Scorecard } from "@/app/lib/definitions";
import { getMyApplications, getMyFeedback, getScorecardsByFeedback } from "@/app/lib/data";
    ...
    async function handleSelectApplication(app: JobApplication) {
        ...
        await Promise.all(
            feedbacksConContenido.map(async (fb) => {
                const scData = await getScorecardsByFeedback(token, fb.id);
